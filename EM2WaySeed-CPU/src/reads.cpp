...
void loadReads(FILE *fp, std::vector<read2Bit_t> &reads2Bit, std::vector<read3Bit_t> &reads3Bit,
	...
	if (r_ctrl == true && size_r > 0) {
		...
		*bytes += size_r;
		...
		tmp2Bit.weSeedInfo.actualSeedCnt = 0;
		for (uint8_t al = 0; al < SEED_NUM; al++){
            tmp2Bit.weSeedInfo.actualExtCnt[al] = 0;
        }
...
uint64_t writeReads(FILE *fp, std::vector<read2Bit_t> &reads, bool is_seeding, char *buffer){
    ...
    for (uint32_t i = 0; i < reads.size(); i++) {
    	if ((is_seeding == true && reads[i].seedAlignedCnt == 0) ||
			(is_seeding == false && reads[i].isaligned_bw == false  && reads[i].isaligned_fw == false)) {
			// write buffer before overflow
			if ((bytes + 512) > BUFF_SIZE) {
				writeFile(fp, buffer, bytes);
				bytes = 0;
