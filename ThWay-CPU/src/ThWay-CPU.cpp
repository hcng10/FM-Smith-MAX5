...
int main(int argc, char *argv[]) {
    ...
    /*openFile(&fmt_fp, s10, "r");
    
    pck_fmt = new uint8_t[CEIL(fmt_len-1, FM_BP_RANGE)];

    for (uint32_t i = 0; i < CEIL(fmt_len-1, FM_BP_RANGE) ; i++){
        size_t sizeread = fread(&pck_fmt[i], 1, sizeof(uint8_t), fmt_fp);
        if (sizeread != sizeof(uint8_t)) {
            fprintf(stderr, "error: unable to read Format file!\n");
            exit(1);
        }
    }

    fclose(fmt_fp);*/
    printf("FINISH ---> Reading Format\n\n");
        ...
        if (!(thread_cnt % 2)) {
        	...
        	loadread_thread = std::thread(loadReads,
        	 ...
        	 if (reads1.size() > 0) {
                ...
                writeSeed_thread = std::thread(seedWrite,
                        sam_fp, 
                        std::ref(reads1),
                        sai,
                        std::ref(chrs),
				...
				/*aligned_cnt1 = aligned_cnt1 + seedWrite(sam_fp, 
                        reads1,
                        sai,
                        chrs,
                        sam_buff);*/
    ...
    printf("Parse and sort time  %.2f s \n", bmk2.parse_time + bmk1.parse_time);
