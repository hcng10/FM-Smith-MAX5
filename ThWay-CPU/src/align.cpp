...
#pragma omp parallel for num_threads(n_threads)
	    for (uint32_t j = 0; j < part_size[i]; j++) {
			...
			uint32_t read_id = id / MISMATCH_DIV;
			uint32_t sub_read_offset = id % MISMATCH_DIV;
			

			if (out[i][j].isaligned_bw == 1 || out[i][j].isaligned_fw == 1 ){

				if (out[i][j].isaligned_bw == 1){
					tmp.low = out[i][j].low_bw;
					tmp.high = out[i][j].high_bw;
					tmp.is_rev_cmpt = false;	
				}
				else{
					tmp.low = out[i][j].low_fw;
					tmp.high = out[i][j].high_fw;
					tmp.is_rev_cmpt = true;			
				}
				tmp.range = tmp.high - tmp.low;
				tmp.seg = sub_read_offset;

				int mtx_n = read_id % 8;
				
				mtx[mtx_n].lock();
				//if (read_id == 1){
				//	cout<<(int)tmp.seg<<" "<<tmp.range<<"\n";
				//}
				reads[read_id].n_seedhits = reads[read_id].n_seedhits + 1;
				reads[read_id].seedhit.push(tmp);
				mtx[mtx_n].unlock();
			}
		}
	}
	...
	bmk->parse_time = bmk->parse_time  + time_taken;
...
void align(std::vector<read2Bit_t> &reads,
	...
	//gettimeofday(&tv1, NULL);
	getPartSize(part_size, reads.size() * MISMATCH_DIV);
...
#pragma omp parallel for num_threads(n_threads)
		for (uint32_t j = 0; j < part_size[i]; j++){
			...
			in[i][j].id = id;
			in[i][j].len = reads[read_id].seq_sub_len[sub_read_offset];
			
			int cpylen = CEIL(reads[read_id].seq_sub_len[sub_read_offset], FM_BP_RANGE);
			memcpy(in[i][j].pck_sym, 
					reads[read_id].pck_sub_sym[sub_read_offset], 
					cpylen);
			in[i][j].is_pad = 0;
