...
void seedWrite(FILE *fp, 
    ...
    for (uint32_t i = 0; i < reads.size(); i++) {
        ...
        if (reads[i].n_seedhits > 0){
            ...
            seedhit_t seed;
            //find the min seedhit based on low/high
            while (reads[i].seedhit.size() > 0){

                seed = reads[i].seedhit.top();
                reads[i].seedhit.pop();

                uint8_t seed_seg = seed.seg;

                uint32_t tmp_val = reads[i].seq_sub_pos[seed_seg];
        
                writeSAMVal(bytes, buffer, &tmp_val, 32);
                buffer[bytes++] = ' ';

                tmp_val = reads[i].seq_sub_len[seed_seg];
        
                writeSAMVal(bytes, buffer, &tmp_val, 32);
                buffer[bytes++] = ' ';


                if (seed.is_rev_cmpt == false){
                    buffer[bytes++] = 'b';
                    buffer[bytes++] = ' ';
                }
                else if (seed.is_rev_cmpt == true){
                    buffer[bytes++] = 'f';
                    buffer[bytes++] = ' ';
                }

                uint16_t tmp_read_cnt = 0;

                uint64_t offset = seed.low;
                for (uint64_t j = 0; j < seed.range && j < EXTEND_PER_SEED ; j++){//
